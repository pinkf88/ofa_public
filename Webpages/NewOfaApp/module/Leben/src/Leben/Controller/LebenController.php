...
class LebenController extends AbstractActionController
{
    ...
    public function indexAction()
    {
        ...
        if ($ortid > 0) {
            $ort = $this->getOrtTable()->getOrt($ortid);
            $select->where('(ortid=' . $ortid . ' OR beschreibung LIKE "%' . $ort . '%" OR bemerkung LIKE "%' . $ort . '%")');
        }
        ...
        $paginator = $this->getLebenTable()->fetchAll($select);
        $paginator->setCurrentPageNumber((int) $this->params()->fromQuery('page', 1));
        $paginator->setItemCountPerPage($countperpage);

        $this->session->offsetSet('page', $this->params()->fromQuery('page', 1));

        $selectform = new LebenSelectForm(
            $this->getLebenTable()->fetchAllYears(),
            $this->getOrtTable()->fetchAll(),
            $this->getLandTable()->fetchAll()
        );
    ...
    public function addAction()
    {
        ...
        $form = new LebenForm($this->getOrtTable()->fetchAll());
        $form->get('submit1')->setValue('Ok');
        $form->get('submit2')->setValue('Ok / Neu');
        $form->get('submit3')->setValue('Ok / Neu+1');

        if ($session->offsetExists('input_datumvon'))
        {
            $form->get('datumvon')->setValue($session->offsetGet('input_datumvon'));
        }

        if ($session->offsetExists('input_datumbis'))
        {
            $form->get('datumbis')->setValue($session->offsetGet('input_datumbis'));
        }
        ...
        if ($request->isPost())
        {
            ...
            if ($form->isValid())
            {
                ...
                $id = $this->getLebenTable()->saveLeben($leben);
                ...
                if ($request->getPost('submit1'))
                {
                    ...
                    return $this->redirect()->toUrl('/leben?page=' . $page);
                }
    ...
    public function editAction()
    {
        ...
        try
        {
            $leben = $this->getLebenTable()->getLeben($id);
        }
        catch (\Exception $ex)
        {
            return $this->redirect()
        ...
        $form->get('ortid')->setValue($leben->ortid);
        ...
        if ($request->isPost())
        {
            ...
            if ($form->isValid())
            {
                ...
                $this->getLebenTable()->saveLeben($leben);
    ...
    public function deleteAction()
    {
        ...
        if (! $id)
        {
            return $this->redirect()->toRoute('leben');
        }
        ...
        if ($request->isPost())
        {
            ...
            if ($del == 'Ja')
            {
                ...
                $this->getLebenTable()->deleteLeben($id);
            }
